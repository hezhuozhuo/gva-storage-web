<template>
  <div>
    <div class="gva-search-box">
      ...
      <el-date-picker v-model="searchInfo.endCreatedAt" type="datetime" placeholder="结束时间"></el-date-picker>
        ...
        <el-form-item label="插件名称">
         <el-input v-model="searchInfo.pluginName" placeholder="搜索条件" />

        </el-form-item>
        <el-form-item label="作者">
         <el-input v-model="searchInfo.pluginAuthor" placeholder="搜索条件" />
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="封面" prop="coverUrl" width="120" />
        <el-table-column align="left" label="插件名称" prop="pluginName" width="120" />
        <el-table-column align="left" label="插件描述" prop="pluginDescribe" width="120" />
        <el-table-column align="left" label="版本号" prop="versionNumber" width="120" />
        <el-table-column align="left" label="作者" prop="pluginAuthor" width="120" />
        <el-table-column align="left" label="设备插件类型" prop="devicePluginType" width="120">
            <template #default="scope">
            {{ filterDict(scope.row.devicePluginType,DEVICE_PLUGIN_TYPEOptions) }}
            </template>
        </el-table-column>
        ...
        <el-table-column align="left" label="按钮组">
            <template #default="scope">
            <el-button type="primary" link icon="edit" size="small" class="table-button" @click="updateTPluginDeviceFunc(scope.row)">变更</el-button>
    ...
    <el-dialog v-model="dialogFormVisible" :before-close="closeDialog" title="弹窗操作">
      <el-form :model="formData" label-position="right" ref="elFormRef" :rules="rule" label-width="80px">
        ...
        <el-form-item label="插件名称:"  prop="pluginName" >
          <el-input v-model="formData.pluginName" :clearable="true"  placeholder="请输入" />
        </el-form-item>
        <el-form-item label="插件描述:"  prop="pluginDescribe" >
          <el-input v-model="formData.pluginDescribe" :clearable="true"  placeholder="请输入" />
        </el-form-item>
        <el-form-item label="版本号:"  prop="versionNumber" >
          <el-input v-model="formData.versionNumber" :clearable="true"  placeholder="请输入" />
        </el-form-item>
        <el-form-item label="作者:"  prop="pluginAuthor" >
          <el-input v-model="formData.pluginAuthor" :clearable="true"  placeholder="请输入" />
        </el-form-item>
        <el-form-item label="设备插件类型:"  prop="devicePluginType" >
          <el-select v-model="formData.devicePluginType" placeholder="请选择" style="width:100%" :clearable="true" >
            <el-option v-for="(item,key) in DEVICE_PLUGIN_TYPEOptions" :key="key" :label="item.label" :value="item.value" />
          </el-select>
        </el-form-item>
...
import {
  createTPluginDevice,
  deleteTPluginDevice,
  deleteTPluginDeviceByIds,
  updateTPluginDevice,
  findTPluginDevice,
  getTPluginDeviceList
} from '@/api/tPluginDevice'
...
// 自动化生成的字典（可能为空）以及字段
const DEVICE_PLUGIN_TYPEOptions = ref([])
...
const getTableData = async() => {
  const table = await getTPluginDeviceList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const setOptions = async () =>{
    DEVICE_PLUGIN_TYPEOptions.value = await getDictFunc('DEVICE_PLUGIN_TYPE')
...
const onDelete = async() => {
      ...
      const res = await deleteTPluginDeviceByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateTPluginDeviceFunc = async(row) => {
    const res = await findTPluginDevice({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
        formData.value = res.data.rePluginDevice
...
// 删除行
const deleteTPluginDeviceFunc = async (row) => {
    const res = await deleteTPluginDevice({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  res = await createTPluginDevice(formData.value)
                  break
                case 'update':
                  res = await updateTPluginDevice(formData.value)
                  break
                default:
                  res = await createTPluginDevice(formData.value)
                  break
