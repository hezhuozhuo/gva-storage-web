<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="规则描述" prop="ruleDescribe" width="120" />
        ...
        <el-table-column align="left" label="按钮组">
            <template #default="scope">
            <el-button type="primary" link icon="edit" size="small" class="table-button" @click="updateTRuleEngineFunc(scope.row)">变更</el-button>
...
const getTableData = async() => {
  const table = await getTRuleEngineList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteTRuleEngineByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateTRuleEngineFunc = async(row) => {
    const res = await findTRuleEngine({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteTRuleEngineFunc = async (row) => {
    const res = await deleteTRuleEngine({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
